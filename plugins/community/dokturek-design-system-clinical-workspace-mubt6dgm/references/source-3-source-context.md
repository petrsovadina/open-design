...
Canonical design-system title: Dokturek.ai builds AI products Design System

Dokturek.ai builds AI products for Czech primary care. Two products: Benjamin,
an AI assistant for general practitioners that handles documentation, triage of
routine admin and lookup during a visit; and SVL.ai, a retrieval system over
Czech clinical guidelines that answers with citations to the source document.
Everything runs on self-hosted, open-source Medplum (headless FHIR EHR), so
patient data stays under the clinic's control and integrates with existing
healthcare standards.

Audience: general practitioners and small clinics in the Czech Republic —
time-poor, sceptical of software that adds clicks, responsible for the outcome.

Voice: plain, precise, unhyped. We speak like a competent colleague, not like a
vendor. We name limits instead of hiding them, we never imply the AI makes the
clinical decision — the doctor does. No superlatives, no "revolution", no
emoji, no fake urgency. Czech first, English where the audience is technical.

## Source Links

- https://github.com/Dokturek-ai/dokturek-platform
...
1. For each linked repository, run the bounded intake command before writing design-system files. The command tries this-device access first (`git clone`, then authenticated GitHub CLI via `gh auth login --web`) and uses the Composio GitHub connector only as a connector-platform fallback.
   - `"$OD_NODE_BIN" "$OD_BIN" tools connectors github-design-context --repo 'https://github.com/Dokturek-ai/dokturek-platform' --output context/github/Dokturek-ai-dokturek-platform.md`
...
- Logo malé velikosti.png
- logo.png
- Bricolage Grotesque.zip
- Patient Card HU.pen
- HU Design.pen
- Poznamky EHR.md
- EHR design image.png
- file-a4f07ec428c0987867856ed496d2006e.jpeg
- file-30a72f8e5753df218e9d2545fdc4a4c8.jpeg

Uploaded brand asset files under `assets/`:
- assets/Logo-male_-velikosti.png
- assets/logo.png
- assets/Bricolage-Grotesque.zip
- assets/Patient-Card-HU.pen
- assets/HU-Design.pen
- assets/Poznamky-EHR.md
- assets/EHR-design-image.png
- assets/file-30a72f8e5753df218e9d2545fdc4a4c8.jpeg
